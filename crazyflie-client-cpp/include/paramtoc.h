/*
* Header-only implementation of the param TOC for crazyflie
*
...
/**
* Holds a ParamTocElement
* and provides an interface to its properties
* for fetching and setting a ParamValue
*/
struct ParamTocElement
{
	...
	*/
	...
	/**
	* Copy Constructor
	* @param the ParamTocElement to copy.
	*/
	ParamTocElement(const ParamTocElement& a)
	{
		ident = a.ident;
		group = a.group;
		name = a.name;
	...
	*/
	ParamTocElement(uint16_t id, uint8_t* data)
	{
		ident = id;
		access = 0;
		if (data != NULL)
		{
			...
			access = data[0] & 0x10;
			extended = ((metadata & 0x10) != 0);

			get_cstring_from_id(metadata & 0x0f, ctype);
			get_unpack_string_from_id(metadata & 0x0f, pytype);
	...
	*/
	static uint8_t get_id_from_cstring(std::string name)
	{
		uint8_t result = gTypeNotFound;
		for (int32_t i = 0; i < gPtTypesSize; i++)
		{
			if (name == ptTypes[i].name)
			{
				result = i;
				break;
			}
	...
	*/
	static bool get_cstring_from_id(uint8_t id, std::string& name)
	{
		bool result = false;
		if (id < gPtTypesSize)
		{
			name = ptTypes[id].name;
			result = true;
		}
		return(result);
	}
	...
	*/
	static bool get_unpack_string_from_id(uint8_t id, std::string& pacName)
	{
		bool result = false;
		if (id < gPtTypesSize)
		{
			pacName = ptTypes[id].pytype;
			result = true;
		}
		return(result);
	...
	*/
	static uint8_t get_size_from_id(uint8_t id)
	{
		...
		if (id < gPtTypesSize)
		{
			result = ptTypes[id].size;
		}
		return(result);
	}
	
	/**
	* Determines of the ctype id is an integer
	* @param The id of the c type.
	* @returns true if it is an integer.
	*/
	static bool is_integer(uint8_t id)
	{
		return(!(id == ptFloat32 ||
				id == ptFloat64));
	}
	...
	/**
	* Determines of the ctype id is signed
	* @param The id of the c type.
	* @returns true if it is signed.
	*/
	static bool is_signed(uint8_t id)
	{
		return(id > ptUint64);
	}

	/**
	* Determines if the ParamTocElement is writable.
	* @returns true if it has read-write access.
	*/
	bool is_writable()
	{
		return(access == RW_ACCESS);
	}

	/**
	* Determines if the ParamTocElement is extended.
	* @returns true if it is extended.
	*/
	bool is_extended()
	{
		return(extended);
	...
	/**
	* Determines if the ParamTocElement is persistent.
	...
	*/
	...
	/**
	* Mark this ParamTocElement persistent.
	*/
	...
	* and for UI binding.
	*/
	beginProperties(ParamTocElement)
		...
		reflectKeyValue(boolr, extended)
		endProperties
};
...
/**
* Holds a named list of ParamTocElements
*/
struct ParamTocGroup
{
	stringr name;
	propVect<ParamTocElement> elements;

	ParamTocGroup() {}
	ParamTocGroup(const ParamTocGroup& a)
	{
		name = a.name;
		elements = a.elements;
	}
	...
	* and for UI binding.
	*/
	beginProperties(ParamTocGroup)
		reflectKeyValue(stringr, name)
		reflectPropVector(propVect<ParamTocElement>, elements)
		endProperties
};


/**
* Holds a list of ParamTocGroups.
* Supports reading and writing based on a crc.
* This allows the LogToc to be cached.
*/
struct ParamToc		//Container for ParamTocElements
{
	uint32_t crc = 0;					/**< The crc for the entire TOC */
	propVect<ParamTocGroup> groups;		/**< The list of ParamTocGroups in the TOC*/
	std::string defaultPath;			/**< The location for reading and writing the cache.*/
	ParamTocElement nullElement;		/**< To return a null element if not found.*/
	...
	*/
	...
	/**
	* Finds the index of the ParamTocGroup with groupName.
	...
	*/
	...
	/**
	* Finds the index of the ParamTocElement within the group at groupDex elementName.
	...
	*/
	int32_t nameIndex(int32_t groupDex, const char* elementName)
	{
		...
		{
			ParamTocGroup& group = groups[groupDex];
	...
	*/
	void add_element(ParamTocElement& element)
	{
	...
	* @returns The found element or nullElement.
	*/
	ParamTocElement& get_element_by_name(std::string completeName)
	{
		...
		if (nameStart != std::string::npos)
		{
			...
			if (groupDex >= 0)
			{
				...
				if (elemDex >= 0)
				{
					return(groups[groupDex].elements[elemDex]);
				}
			}
		}
		return(nullElement);
	...
	*/
	uint16_t get_element_id(std::string completeName)
	{
		...
		ParamTocElement& element = get_element_by_name(completeName);
		id = element.ident;
		return(id);
	}
	...
	* @returns element identifier or NO_IDENT.
	*/
	ParamTocElement& get_element_by_complete_name(std::string completeName)
	{
		return(get_element_by_name(completeName));
	}

	/**
	* Get the element identifier by group name and element name
	* @param The group name to find.
	* @param The element name to find.
	* @returns The found element or nullElement.
	*/
	ParamTocElement& get_element(std::string groupName, std::string elemName)
	{
		bool result = false;
		int32_t groupDex = groupIndex(groupName.c_str());
		if (groupDex >= 0)
	...
	/**
	* Gets the total number of elements in the TOC.
	* @returns The number of elements in the TOC.
	*/
	size_t get_id_count()
	{
		size_t result = 0;
	...
	* @returns The found element or nullElement.
	*/
	ParamTocElement &get_element_by_id(uint16_t ident)
	{
		for (size_t i = 0; i < groups.size(); i++)
		{
	...
	*/
	bool write(uint32_t crc)
	{
		...
		if (getfullTocPath(crc, fullPath))
		{
			...
			if (result)
			{
				messageOut << "Wrote the Param TOC to: ";
				messageOut << fullPath;
				messageOut << "\n\r";
			}
			else
			{
				messageOut << "Could not write the Param TOC\n\r ";
			}
		}
		return(result);
	}
	...
	* and for UI binding.
	*/
	beginProperties(ParamToc)
		reflectPropVector(propVect<ParamTocGroup>, groups)
		endProperties
};
